...
const ExistServiceRequestRespond = ({ onClose, isOpen, request }) => {
    ...
    const handleSubmit = (e) => {
        ...
        post(
            route("service.requests.respond.accept", { id: request.id }),
            {
                onSuccess: () => onClose(),
            }
        );
            ...
            <DialogContent className=" max-w-3xl overflow-y-scroll max-h-screen">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="space-y-4">
                         ...
                         {/* Accept Title Answer */}
                         <div>
                            <Label htmlFor="lab_answer_title">
                                {language === "en"
                                    ? "Service Title Answer"
                                    : language === "ar"
                                    ? "إجابة قبول الخدمة"
                            ...
                            />
                            <InputError
                                message={errors.lab_answer_title}
                                className="mt-2"
                            />
                        </div>
                        {/* Accept Answer */}
                        ...
                        {/* Price */}
                        <div>
                            <Label htmlFor="lab_answer_price">
                                {language === "en"
                                    ? "Service Price"
                                    : language === "ar"
                                    ? "سعر الخدمة"
                            ...
                            <Input
                                id="lab_answer_price"
                                type="number"
                                value={data.lab_answer_price}
                                onChange={(e) =>
                                    setData("lab_answer_price", e.target.value)
                                }
                                className="w-full"
                                placeholder={
                                    language === "en"
                            ...
                            />
                            <InputError
                                message={errors.lab_answer_price}
                                className="mt-2"
                            />
                        </div>
