...
const EditThesisModal = ({ onClose, thesis }) => {
    ...
    const { data, setData, put, processing, errors } = useForm({
        supervisor_remarks: JSON.parse(thesis.supervisor_remarks || '[]'),
    });

    const [remarks, setRemarks] = useState(() => {
        return Array.isArray(data.supervisor_remarks)
    ...
    const handleRemoveRemark = (index) => {
        ...
        const updatedData = [...data.supervisor_remarks];
        updatedData.splice(index, 1);
        setData("supervisor_remarks", updatedData);
    };
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(
            route("supervisor.space.supervisor.remark.edit", { id: thesis.id }),
            {
                onSuccess: () => onClose(),
            }
        );
