...
const TeamMembersTable = ({
    ...
    const externalAuthors = sc.external_author ? JSON.parse(sc.external_author) : [];
        ...
        <Card className="border border-gray-300 shadow-lg rounded-lg bg-white overflow-hidden">
            ...
            <CardContent className="p-6">
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            <TableCell className="font-medium text-gray-600">#</TableCell>
                            <TableCell className="font-medium text-gray-600">Name</TableCell>
                            <TableCell className="font-medium text-gray-600">Email</TableCell>
                            <TableCell className="font-medium text-gray-600">Action</TableCell>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {/* Display Lab Members */}
                        {members.map((member) => (
                            <TableRow key={member.id} className="hover:bg-gray-50">
                                <TableCell className="p-4">
                                    <Avatar user={member} className="w-10 h-10 rounded-full" />
                                </TableCell>
                        ...
                        {externalAuthors.map((author, index) => (
                            <TableRow key={index} className="hover:bg-gray-50">
                                ...
                                <TableCell className="p-4 text-red-500 flex items-center space-x-2">
                                    <FiTrash2
                                        className="w-5 h-5 cursor-pointer hover:text-red-700"
                                        onClick={() => openNoDeleteModal(author)}
                                    />
                                </TableCell>
                            </TableRow>
                        ))}
