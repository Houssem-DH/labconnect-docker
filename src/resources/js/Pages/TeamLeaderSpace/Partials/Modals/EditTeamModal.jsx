...
const EditTeamModal = ({ onClose, isOpen, team }) => {
    const { data, setData, put, processing, errors, reset } = useForm({
        title: team.title,
        acronym_team_name: team.acronym_team_name,
        Keywords: team.Keywords,
        theme_description: team.theme_description,
        sub_research_area: JSON.parse(team.sub_research_area || '[]'),
    ...
    const [subResearchArea, setSubResearchArea] = useState(() => {
        console.log("Initializing sub_research_area:", data.sub_research_area);
        return Array.isArray(data.sub_research_area) ? data.sub_research_area : [];
    });
    ...
    const submit = (e) => {
        e.preventDefault();
        put(route("team.leader.team.edit", { id: team.id }), {
            onSuccess: () => {
                onClose();
            },
            onError: (errors) => {
